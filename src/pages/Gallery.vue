<template>
  <div class="">
    ...
    <div class="px-2 sm:px-4 lg:px-10 xl:px-16 mx-auto py-10">
      <div
        v-if="isLogin && user_type == 'Artisan' && currentRouteParam == null"
        class="w-full flex pt-2 justify-center"
      >
        <svg
          class="h-9 w-9 pr-2 text-yellow-600"
      ...
      <div class="">
        <section id="photos" class="flex flex-wrap">
          <div
            v-if="galleryData == '' || galleryData == [] || galleryData == null"
            class="text-gray-400 mt-28 text-xl font-bold w-full flex justify-center items-center"
          >
            No Gallery Found
          </div>
          <div
            v-else
            class="my-3 px-1 w-full sm:w-1/2 md:w-1/3 lg:my-4 lg:w-1/4"
            v-for="gallery_detail in galleryData"
            :key="gallery_detail.id"
          >
            <!-- Image -->

            <div
              @mouseover="mouse_Over(gallery_detail)"
              @click="mouse_Over(gallery_detail)"
              class="overflow-hidden relative shadow-lg"
            >
              <img
                alt="Placeholder"
                class="block object-cover opacity-90 h-44 w-full"
                :src="gallery_detail.photos"
              />

              <div
                v-if="mouseOver == gallery_detail"
                class="bg-opacity-70 flex bg-black items-center absolute justify-center p-1 top-0 h-full w-full"
              >
                <span>
                  <svg
                ...
                <svg
                  v-if="
                    ...
                    gallery_detail.user_id == user_id
                  "
                  @click="openModal('Edit Image', gallery_detail.id)"
                  class="h-8 w-8 text-yellow-600"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                ...
                <svg
                  v-if="
                    ...
                    gallery_detail.user_id == user_id
                  "
                  @click="deleteGallery(gallery_detail.id)"
                  class="h-8 w-8 text-yellow-600"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
